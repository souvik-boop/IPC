    ...
    public partial class Form1 : Form
    {
        ...
        protected override void WndProc(ref Message m)
        {
            if (m.Msg == NativeMethods.WM_COPYDATA)
            {
                ...
                if (dataType == 2)
                {
                    string fileName = Marshal.PtrToStringUni(copyData.lpData);

                    // Add the file name to the edit box
                    richTextBox1.AppendText(fileName);
                    richTextBox1.AppendText("\r\n");

                    using (StreamWriter writer = File.AppendText("end-time"))
                    {
                        writer.WriteLine(DateTime.Now.ToString("dddd, dd MMMM yyyy HH:mm:ss:fffffff"));
                    }
                }
                else
                {
